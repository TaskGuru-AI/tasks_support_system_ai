...
- Запустить `poetry scripts/prefetch_data.py`
- Получить данные `just pull-data` или у товарища p2p
- Придерживаемся trunk-based development. Ветки короткоживущие, не делаем merge коммитов. PR мерджить желательно через Squash & Merge.
