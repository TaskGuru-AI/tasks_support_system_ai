...
def create_anomaly_feature(
    df, queue_id_col="queue_id", target_col="new_tickets", contamination=0.05, random_state=42
):
    """
    ...
    for lag in lags:
        for window in windows:
            for stat in statistics:
                ...
                if stat == "mean":
                    df[col_name] = (
                        df.groupby(queue_id_col)[f"{target_col}_lag_{lag}"]
...
class GlobalPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_ = create_lag_features(
            X_, lags=self.lags, queue_id_col=self.queue_id_col, target_col=self.target_col
        )
...
def create_preprocessing_pipeline(
    ...
    """
    global_preprocessor = GlobalPreprocessor(
        lags=lags_to_create,
