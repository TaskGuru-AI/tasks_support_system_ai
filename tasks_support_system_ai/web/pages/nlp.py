...
nlp_predict_model_selector = tab3.selectbox(
    label="Choose model:",
    placeholder="Choose model",
    options=["model_id_1", "model_id_2"],
    index=None,
)  # This should be the list of models

nlp_input_selector = tab3.radio(
    label="Text or CSV file:", options=["Text", "CSV"], index=0, horizontal=True
)

# Kept both text input and file selector as user may decide mid-action they need them

nlp_predict_input_text = tab3.text_area(
    label="Text:", help="Please provide text of the conversation"
)

nlp_predict_input_file_uploader = tab3.file_uploader(
    label="CSV:", type=["csv", "py"], help="Please provide a CSV file"
)

nlp_predict_button = tab3.button(
    label=f"Predict from {nlp_input_selector}",
    type="primary",
...
nlp_clear_form_button = tab3.button(label="Clear form", type="secondary")

nlp_prediction_results = tab3.table(
    pd.DataFrame(
        {
